...
match(W,omega(R))    :- match(W1,R), concat(W1,W2,W); match(W2,omega(R)). 
...
test_false05 :- W0=[0|W0], solve(match([1,1,0,1|W0],cat(star(1),omega(0)))).
test_false05.

test_false06 :- W01=[0,1|W01], solve(match(W01,star(cat(omega(plus(eps,0)),1)))). %% counter-example for initially incorrect version
test_false06.

test_false07 :- W0=[0|W0], solve(match(W0,omega(cat(star(0),1)))).
test_false07.
